...
STATUS_TEXT = {
    ...
    "supported": "SUPPORTED, not proven. Every critical prediction passed. Next: a more severe test, or a system outside the current scope.",
    ...
    "mixed": "MIXED. Critical predictions passed, at least one non-critical prediction failed. The theory survives; the failures are anomalies and become predictions for the next version.",
...
def compute(study: dict) -> dict:
    ...
    log10_evidence = 0.0
    rows, missing = [], []
    crit_fail = crit_incl = noncrit_fail = 0
    for p in preds:
        o = p.get("outcome")
        if o is None:
            missing.append(p["id"])
        ...
        lr_pass, lr_fail = likelihood_ratios(p)
        lr = {"pass": lr_pass, "fail": lr_fail, "inconclusive": 1.0}[o]
...
def report(study: dict, v: dict, exploratory: bool = False) -> str:
    ...
    lines.append(f"credence {v['prior_credence']:.2f} -> {v['posterior_credence']:.2f}   "
                 f"(log10 evidence {v['log10_evidence']:+.2f}; likelihood ratios multiplied as if independent)")
    ...
    if v["status"] == "mixed":
        ...
        lines.append("anomalies to carry forward: " + ", ".join(failed))
    return "\n".join(lines)
